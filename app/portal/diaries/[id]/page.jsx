...
const DailyDiary = () => {
  ...
  const onCoordinatorApprove = async () => {
    await api
      .put(`DailyDiary/${id}/Coordinator/Approve`, null, token)
      .then(() => {
        setDailyDiary((prev) => ({
          ...prev,
          coordinatorApprovalStatus: 2,
        }));
      });
  };
    ...
    <>
            ...
            <div className="flex justify-between">
              ...
              <p className="py-1 px-2 font-semibold bg-light-blue-background w-fit">
                ...
                <span className="text-red">
                  {dateLocalizer(dailyDiary.deadline)}
                </span>{" "}
              </p>
              {dateLocalizer(dailyDiary.dateSubmitted) >
                dateLocalizer(dailyDiary.deadline) && (
                <p className="text-red text-sm font-semibold self-center">
                  (Late submission)
                </p>
              )}
            </div>
            ...
            <div className="flex gap-20">
              <div className="flex gap-2">
                ...
                {dailyDiary.supervisorApprovalStatus == 0 && (
                  <Status name={"Waiting"} color={"red"} />
                )}
                {dailyDiary.supervisorApprovalStatus == 1 && (
                  <Status name={"Requested"} color={"blue"} />
                )}
              ...
              <div className="flex gap-2">
                ...
                {dailyDiary.coordinatorApprovalStatus == 1 && (
                  <Status name={"Requested"} color={"green"} />
                )}
                {dailyDiary.coordinatorApprovalStatus == 2 && (
                  <Status name={"Approved"} color={"red"} />
                )}
              </div>
            </div>
            <div className="w-fit">
      ...
      <div className=" w-full bg-white shadow font-default mt-2 p-2 mb-10">
        ...
        <div className="flex gap-4">
          <Button
            className="mt-4"
            onClick={() => router.push("/portal/diaries")}
          >
            Cancel
          </Button>
        </div>
