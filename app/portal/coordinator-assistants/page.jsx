...
const Users = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      await api
        .get(
          `Faculty/${facultyId}/CoordinatorAssistants`,
          {
  ...
  const createUser = async (values) => {
    await api.post(
      `/Faculty/${facultyId}/CoordinatorAssistant`,
      { ...values },
      token
    );
  ...
  const deleteUser = async () => {
    await api.delete(
      `CoordinatorAssistant/${userDeleteModalDetails.userId}`,
      token
    );
    await fetchData();
  ...
  const editOnClickHandler = async (id) => {
    try {
      await api
        .get(`CoordinatorAssistant/${id}`, null, token)
        .then((response) => {
          console.log(`response ${response.firstName}`);
          setUserEditModalDetails({
            isOpen: true,
            userId: response.coordinatorAssistant.id,
            firstName: response.coordinatorAssistant.firstName,
            lastName: response.coordinatorAssistant.lastName,
            email: response.coordinatorAssistant.email,
          });
        });
    } catch (error) {
      throw error;
  ...
  const updateUser = async (values) => {
    ...
    if (values["new-password"]) {
      await api.put(
        `CoordinatorAssistant/${userEditModalDetails.userId}/Password`,
        {
    ...
    await api.put(
      `CoordinatorAssistant/${userEditModalDetails.userId}`,
      {
  ...
  return (
    <div className=" font-default text-dark-blue">
      ...
      <CreateFormModal
        ...
        onCreate={createUser}
        title={"Create Coordinator Assistant"}
        width={500}
      >
        <Row gutter={16}>
          <Col span={12}>
      ...
      <UpdateFormModal
        ...
        onUpdate={updateUser}
        title={"Update Coordinator Assistant"}
        initialValues={{
